        ...
        <motion.div initial="hidden" animate="visible" variants={fadeUp} custom={1} className="flex flex-col gap-3">
          <div className="relative max-w-sm">
            ...
            <Input
              ...
              className="pl-10 font-body font-light h-10"
            />
          </div>
          <div className="flex gap-[1px] bg-border overflow-x-auto">
            {(["all", "active", "at_risk", "inactive"] as const).map((f) => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className={`px-3 py-2 text-editorial-sm text-[9px] whitespace-nowrap transition-colors duration-300 ${
                  filter === f ? "bg-foreground text-primary-foreground" : "bg-background text-muted-foreground hover:text-foreground"
                }`}
              >
            ...
            <div className="text-center py-16">
              ...
              {students?.length === 0 && (
                <p className="text-xs text-muted-foreground font-body font-light mt-2">
                  Seus alunos aparecerão aqui quando se cadastrarem e vincularem a você.
                </p>
              )}
            </div>
          ) : (
            <div className="space-y-0">
              {filtered.map((student, i) => {
                ...
                return (
                  <div
                    key={student.id}
                    className="flex items-center gap-3 py-4 border-b border-border hover:bg-accent/30 transition-colors duration-300 px-2 -mx-2 cursor-pointer group"
                  >
                    <div className="w-9 h-9 rounded-full bg-accent flex items-center justify-center shrink-0">
                      <span className="text-xs font-display">{student.full_name.charAt(0).toUpperCase()}</span>
                    </div>
                    ...
                    <span className={`status-badge ${sc.class} hidden sm:inline-block`}>{sc.label}</span>

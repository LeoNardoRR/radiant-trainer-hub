        ...
        <motion.div initial="hidden" animate="visible" variants={fadeUp} custom={0} className="flex items-start justify-between gap-4">
          ...
          {unread.length > 0 && (
            <button
              ...
              className="text-editorial-sm text-muted-foreground hover:text-foreground transition-colors text-[9px] min-h-[44px] px-3 flex items-center shrink-0"
            >
                  ...
                  {unread.map((notif) => {
                    ...
                    return (
                      <div
                        ...
                        className="flex gap-3 py-4 border-t border-border hover:bg-accent/30 active:bg-accent/50 transition-colors duration-300 px-3 -mx-3 cursor-pointer min-h-[64px]"
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[11px] font-body font-light text-muted-foreground mt-0.5">{notif.message}</p>
                        </div>
              ...
              <motion.div initial="hidden" animate="visible" variants={fadeUp} custom={3}>
                ...
                <div className="space-y-0">
                  {read.map((notif) => {
                    const Icon = iconMap[notif.type] || Bell;
                    return (
                      <div key={notif.id} className="flex gap-3 py-4 border-t border-border px-3 -mx-3 opacity-50 min-h-[64px]">

...
import { Send, ArrowLeft, MessageSquare, Check, CheckCheck, Plus, Search, Megaphone, X, Loader2, Users, Paperclip } from "lucide-react";
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      await sendMessage.mutateAsync({
        ...
        content: file.type.startsWith('video/') ? "Enviou um vídeo" : "Enviou uma imagem",
        fileUrl: publicUrl,
        fileType: file.type
      });
  ...
  return (
    <AppLayout>
      <div className="space-y-4">
        ...
        >
          <div className={`border-r border-border overflow-y-auto ${!showConversations && selectedPartner ? "hidden md:block" : "block"}`}>
            <div className="p-3 border-b border-border">
              <button onClick={() => setShowNewChat(!showNewChat)} className="w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl bg-primary/10 text-primary text-sm font-medium hover:bg-primary/15 transition-colors min-h-[44px]">
              ...
              <div className="border-b border-border bg-muted/30">
                <div className="p-3">
                  <div className="relative">
                    ...
                    <Input value={searchContact} onChange={(e) => setSearchContact(e.target.value)} placeholder={role === "trainer" ? "Buscar aluno..." : "Buscar personal..."} className="pl-10 h-10 text-sm rounded-xl" />
                  </div>
                </div>
                {availableContacts.length === 0 ? (
                  <p className="text-xs text-muted-foreground text-center py-3 px-3">Nenhum contato disponível</p>
                ) : (
                  availableContacts.map((contact) => (
                    <button key={contact.id} onClick={() => handleSelectConversation(contact.id, contact.name)} className="w-full text-left p-3 px-4 hover:bg-accent/50 transition-colors flex items-center gap-3 min-h-[52px]">
            ...
            {isLoadingConversations ? (
              ...
              conversations.map((conv) => (
                <button key={conv.partnerId} onClick={() => handleSelectConversation(conv.partnerId, conv.partnerName)} className={`w-full text-left p-4 border-b border-border transition-all duration-200 min-h-[68px] ${selectedPartner === conv.partnerId ? "bg-accent/50" : "hover:bg-accent/30"}`}>
                  <div className="flex items-center gap-3">
                    <div className="relative shrink-0">
                      <div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center">
                        <span className="text-sm font-display font-bold text-primary">{conv.partnerName.charAt(0)}</span>
                      </div>
                      {conv.unread && <div className="absolute -top-0.5 -right-0.5 w-3 h-3 rounded-full bg-primary ring-2 ring-card" />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between">
                        <p className={`font-body text-sm truncate ${conv.unread ? "font-semibold" : "font-medium"}`}>{conv.partnerName}</p>
                        <span className="text-[10px] text-muted-foreground font-body shrink-0 ml-2">{formatDistanceToNow(new Date(conv.time), { addSuffix: false, locale: ptBR })}</span>
                      </div>
                      <p className="text-[11px] font-body truncate text-muted-foreground">{conv.lastMessage}</p>
                    </div>
                  </div>
                </button>
              ))
            ...
            {selectedPartner ? (
              <>
                <div className="p-3 border-b border-border flex items-center gap-3 min-h-[56px] bg-background/80 backdrop-blur-sm">
                  <button onClick={() => { setShowConversations(true); setSelectedPartner(null); }} className="md:hidden p-2 min-h-[44px] min-w-[44px] flex items-center justify-center hover:bg-accent rounded-xl transition-colors">
                ...
                <div className="flex-1 p-4 space-y-1 overflow-y-auto bg-muted/10" onScroll={handleScroll}>
                  {Object.entries(groupedMessages).map(([date, msgs]) => (
                    <div key={date}>
                      ...
                      {msgs?.map((msg) => (
                        <div key={msg.id} className={`flex mb-1.5 ${msg.sender_id === user?.id ? "justify-end" : "justify-start"}`}>
                          <div className={`max-w-[80%] p-3 ${msg.sender_id === user?.id ? "bg-primary text-primary-foreground rounded-2xl rounded-br-md" : "bg-card border border-border rounded-2xl rounded-bl-md"}`}>
                            {msg.file_url && (
                              <div className="mb-2 rounded-lg overflow-hidden">
                                {msg.file_type?.startsWith('video/') ? (
                                  <video src={msg.file_url} controls className="max-w-[200px]" />
                                ) : (
                                  <img src={msg.file_url} alt="Media" className="max-w-[200px] cursor-pointer" onClick={() => window.open(msg.file_url, '_blank')} />
                                )}
                              </div>
                            )}
                ...
                <div className="p-3 border-t border-border bg-background" style={{ paddingBottom: "max(1rem, env(safe-area-inset-bottom))" }}>
                  <div className="flex gap-2 items-center">
                    ...
                    <button onClick={() => fileInputRef.current?.click()} disabled={isUploading} className="p-3 hover:bg-muted rounded-2xl transition-colors min-h-[48px] min-w-[48px] flex items-center justify-center text-muted-foreground disabled:opacity-50">
                      {isUploading ? <Loader2 className="h-5 w-5 animate-spin" /> : <Paperclip className="h-5 w-5" />}
                    </button>
                    <Input value={message} onChange={(e) => setMessage(e.target.value)} onKeyDown={(e) => e.key === "Enter" && !e.shiftKey && handleSend()} disabled={isUploading} placeholder="Escreva uma mensagem..." className="font-body h-12 rounded-2xl bg-muted/20" />
                    <button onClick={handleSend} disabled={(!message.trim() && !isUploading) || isUploading} className="p-3 bg-primary text-primary-foreground rounded-2xl hover:bg-primary/90 transition-all min-h-[48px] min-w-[48px] flex items-center justify-center shadow-lg shadow-primary/20 disabled:opacity-40 active:scale-95">
